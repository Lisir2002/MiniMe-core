...
/**
 * 菜单体系 · 唯一推荐实现（基于 M3 `DropdownMenu` 封装）。
 *
 * 本文件是 newui 菜单体系的**唯一推荐入口**，统一封装 Material3 [DropdownMenu]：
 *  - [AppMenu]：菜单容器；
 *  - [AppMenuItem]：普通菜单项（图标 + 标题 + 尾随）；
 *  - [AppSelectMenuItem]：单选 / 复选菜单项（radio / 对勾尾随）；
 *  - [AppMenuHeader]：分组标题；
 *  - [AppMenuDivider]：分隔线。
 *
 * 已废弃的旧实现见 AppMenus.kt（[AppActionSheet] / [AppActionSheetItem] / [AppSelectField]），
 * 请勿在新代码中引用。
 */
...
fun AppMenu(
    ...
    DropdownMenu(
        ...
        tonalElevation = AppElevation.Z4,
