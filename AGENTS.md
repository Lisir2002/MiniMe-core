...
> ⚠️ **本规约为发版最高优先级约束。任何 AI / 维护者推 Tag 发版前，必须逐条核对以下全部规则；违反任一条即视为发版失败，必须修正后重发。**
...
**只保留用户层。** 不再维护开发者层（Keep a Changelog 六类）和大模型层（AGENTS.md 变更登记）。所有发版说明统一用用户语言撰写，载体为 GitHub Release 正文。
...
## ⚠️ 已知问题
- {问题描述}：{临时规避方法}，预计{版本/时间}修复
- （无已知问题则写「无」）

## 📦 安装包
- [{安装包文件名}]({下载链接})（{大小}）

**完整更新历史**：{compare 链接，如 https://github.com/.../compare/v0.0.0.14...v0.0.0.15}
```

**写作铁律（逐条核对，违反即重写）：**

1. **利益优先**：每条先说「用户能做什么」，不说「我们做了什么」。✅"你可以为每个模型单独调节温度" ❌"新增模型级 temperature 覆盖"
2. **平实语言**：禁止内部术语、类名、函数名、表名、ticket号、commit hash、migration 编号。
3. **每条一个变更**：禁止「各种改进」「多项优化」「修复若干bug」等模糊表述。
4. **单条≤一行**：每条不超过80字，不换行。
5. **描述症状而非代码**：修复项写用户遇到的问题，不写技术根因。✅"修复了删除供应商后设置页面偶发闪退" ❌"修复 deleteProvider 未级联清理 sampling_config 导致 NPE"
6. **最新优先**：版本倒序排列，最新版本在最上方。
7. **破坏性变更醒目标注**：用 ⚠️ 开头，并明确说明用户需要做什么操作。
