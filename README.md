...
1. 前往 [Releases 页面](https://github.com/Lisir2002/MiniMe-core/releases)，下载最新版本 APK（`minime-<tag>.apk`，单包通用：真机与模拟器同一安装包）。
...
- **正式签名**：签名密钥已入库（`app/minime.jks` + `app/keystore.properties`），CI 构建时直接使用，无需额外配置 Secrets。
- **实时监控与产物校验**、完整命令与 CI job 详解：见 [docs/ci-release.md](./docs/ci-release.md)（云端构建发版运维手册）。
- **Release 页面**：https://github.com/Lisir2002/MiniMe-core/releases
...
| 数据库 | SQLDelight 2.2.1（4 库拓扑：agent / credentials / workspace / aux）+ SQLCipher 全盘加密（Keystore 密钥）+ 凭据字段级 AES-GCM；旧 Room 已完全移除 |
...
**MiniMe-core 为原创项目**，全部代码由本项目团队独立设计与开发，未基于任何现有项目 fork 或衍生。

## 致谢

以下开源项目在技术选型与组件复用层面提供了参考，特此致谢（本项目代码均为独立实现，未直接引用上述项目源码）：

- [Termux](https://github.com/termux/termux-app) — Android 终端模拟器与 PRoot 方案的技术参考
- [OpenCode](https://github.com/anomalyco/opencode) — 终端 AI 编码工具的产品形态参考
- [Kelivo](https://github.com/Chevey339/kelivo) — 跨平台 LLM 聊天客户端的界面设计参考
