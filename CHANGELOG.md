...
## v0.0.0.2-rc69 — 2026-09-19

### 新功能
- 工作台与会话绑定：ChatSession 新增 workspaceId，新建会话自动绑定当前工作台，历史会话按工作台过滤，运行中会话锁定工作台，删除工作台前提示；
- 对话页背景迁 newui token。

### 修复
- 思考过程完成后不消失，折叠保留；
- 唤起输入法时输入框与键盘之间大块空白（AppShell 键盘可见时不渲染底栏）；
- 会话统计改为单行小字。

## v0.0.0.2-rc68 — 2026-09-18

### 新功能
- 工具链折叠卡：同一 AI 消息组内连续多条工具调用自动合并为一张折叠卡，折叠态单行显示「调用 N 个工具 · X.Xs」，点按展开逐条列出工具名与状态点；组内任一工具失败时默认展开并以红色描边 / 红字提示；单工具仍各自走 AppToolCallCard / AppTerminalLog，互不冲突；
- 终端卡 AppTerminalLog：新增行语义（命令 / 标准输出 / 错误 / 告警）与运行 / 成功 / 失败三态状态点，命令行 `$` 前缀三级高亮，错误整行浅底标红；
- 单工具卡 AppToolCallCard：整卡可折叠（成功默认折叠、失败默认展开），头部箭头旋转动画，命令 / 入参 / 结果统一收纳在一层展开动画内。

## v0.0.0.2-rc67 — 2026-09-18

### 新功能
- 内置浏览器 UI 重写为 iOS Safari 风格（pill 地址栏、底部工具栏、加载进度条）；
- 技能选择底部弹窗接入 newui；
- bash/shell 工具走 AppTerminalLog 终端卡渲染；
- 办公按钮单击切页、长按扇形，图标文字跟随，选择持久化。

### 修复
- 用户气泡右对齐紧凑、工具卡贴边、首条思考过程隐藏、头像边距等 11 项；
- 任务清单吸附条、审批/提问吸附提示、统一返回逻辑。

## v0.0.0.2-rc66 — 2026-09-18

### 新功能
- 对话流 UI 大改版：用户气泡右对齐紧凑、AI 气泡贴头像、工具卡边距/行高/合并；
- 工具调用折叠链（连续工具合并）、流式正文 >3 行折叠、工具入参/结果 >10 行折叠、空态文案；
- 思考块默认显示、>2 行折叠；
- 输入框上方常驻条：任务只读条、提问单题左右切换、审批纯提示（点跳消息+闪烁）；
- 办公扇形菜单扩为浏览器/终端/Git/待开发，内置浏览器重写为 iOS Safari 风 newui；
- 办公按钮：单击进上次页/长按扇形、图标随页、持久化、无历史显示"长按待选"、办公页二次返回退出；
- AppShell 统一 BackHandler 返回。
...
### 新功能
- **新 UI 门户底栏三 tab**：首页改用 newui AppShell 统一装配，底部固定三项——「对话」（左）、「办公」（中央凸起的主色大按钮，比两侧高、向上凸起）、「设置」（右）。对话页与设置页在同一壳内原地切换，不再以二级路由 push。
- **办公扇形轮盘**：点中央「办公」按钮，在底栏上方弹出底部半圆扇形轮盘，沿圆弧从左到右排列「浏览器 / 终端 / 待开发」；「待开发」灰显不可点。再点一次办公按钮或点空白处收起。选中浏览器 / 终端后内容区原地切到对应工具占位页，中央按钮文字同步变为「浏览器 / 终端」；切回对话或设置时恢复「办公」。
- **浏览器 / 终端占位页**：两个新工具页先以居中标题 + 空状态文案占位（「浏览器 · 占位页，待接入」/「终端 · 占位页，待接入」），均用 AppShell 包壳并走设计令牌，暂不接入真实业务。

### 改进
- **设置页升级为门户第三 tab**：点底栏「设置」直接在内容区打开设置，不再从会话抽屉 push；设置页自带顶栏在新壳下不再重复渲染。
- **移除对话侧边栏底部「设置」入口**：会话抽屉底部只保留主题切换按钮，设置统一走底栏第三 tab；抽屉相关接线随之清理。
...
## v0.0.0.2-rc61 — 2026-09-18

### 新功能
- **对话流 Markdown 数学公式**：行内 `$...$` 紫色等宽样式，块级 `$$...$$` 灰底公式块（KaTeX 二期接入）；演示页新增全功能 Markdown+LaTeX 样例消息。
...
### 改进
- **文件图标再扩**：CSS/PHP/Ruby/C/C++/Swift/Lua 接入官方品牌 SVG 矢量。
...
### 新功能
- **图标样板补系统/设置类**：新增分区覆盖深色/浅色模式、外观、通知、锁、密钥、安全、防护、设备、内存、仪表盘、云、书签、收藏、标记、标签、图表、文档、菜单、电源，对齐旧版设置页图标。
