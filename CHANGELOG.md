...
> 工作台与对话绑定修复 + AI 回复消息持久化加固版本：修复删除工作台不级联删除对话、备份恢复丢失绑定关系等严重 Bug；同时为 AI 回复增加三层持久化防护（流式定期落库 + NonCancellable + finally 兜底），彻底解决应用被杀导致回复消息丢失的问题。

### Fixed

- `[工作台]` 删除工作台时级联删除绑定的对话及全部消息：删除确认框显示将同时删除的对话数量，提示"不可恢复"，避免产生数据库中存在但任何工作台都不可见的幽灵会话
- `[工作台]` 备份恢复时保留会话原始 workspacePath 绑定关系：仅当原始路径为空时才兜底到当前工作台，不再强制全部覆盖
- `[工作台]` 工作台详情页"绑定对话"列表正确显示：boundSessionsForWorkspace 回调已接线，可查看每个工作台绑定的对话
- `[工作台]` 切换工作台时自动选择该工作台下的最近会话，增加竞态保护：快速切换时旧查询结果不会覆盖新会话 ID
- `[对话]` AI 回复流式期间定期持久化：每累积 2000 字符或间隔 3 秒执行一次 INSERT OR REPLACE，应用被杀时最多丢失最后几秒内容而非整条回复
- `[对话]` persist 数据库写入使用 NonCancellable 上下文：协程取消时已启动的 IO 写入不会中断，所有 persist 调用点自动受益
- `[对话]` 流式异常中断时 finally 兜底保存已收到的内容：处理流式刚开始就异常终止的极端情况，正常完成时不重复落库

### Security

- `[对话]` 三层持久化防护相互搭配：定期落库（主力）+ NonCancellable（保障）+ finally 兜底（最后防线），彻底解决 AI 回复因应用被杀或协程取消而丢失的问题

## [0.0.0.11] - 2026-09-23

> 数据安全与工作台绑定修复版本：修复删除工作台后关联会话变幽灵会话、备份恢复覆盖绑定关系、工作台详情绑定对话为空、工作台切换竞态等 4 项绑定问题；AI 回复消息持久化三重加固（流式期间定期落库 + NonCancellable 保障 + finally 兜底），彻底解决应用被杀导致回复丢失的问题。

### Fixed

- `[data]` 修复删除工作台后关联会话变幽灵会话：删除工作台时级联删除该工作台下所有会话及全部消息（12 张表事务），删除前弹出确认框显示将删除的对话数量，不可恢复
- `[data]` 修复备份恢复时所有会话绑定关系被覆盖：恢复时保留备份中记录的原始 workspacePath，不再强制覆盖为当前工作台路径
- `[ui]` 修复工作台详情「绑定对话」永远为空：MainActivity 接入 boundSessionsForWorkspace 回调，点击工作台行可展开查看该工作台绑定的所有对话
- `[data]` 修复工作台快速切换时当前会话竞态：collectLatest 内查询完成后校验当前工作台是否仍匹配，不一致则丢弃结果，避免旧查询覆盖新会话
- `[data]` AI 回复消息持久化三重加固：流式输出期间每 2000 字符或 3 秒定期落库（INSERT OR REPLACE），persist 包裹 NonCancellable 防止协程取消中断 IO，finally 块兜底保存已收到内容；应用被杀最多丢最后几秒内容，不再整条丢失

### Security

- `[data]` 删除工作台级联删除关联会话及消息，避免幽灵会话数据残留
