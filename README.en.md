...
<p align="center">
  ...
  <a href="https://github.com/Lisir2002/MiniMe-core/actions"><img src="https://github.com/Lisir2002/MiniMe-core/actions/workflows/ci.yml/badge.svg" alt="CI Status" /></a>
...
1. Go to the [Releases page](https://github.com/Lisir2002/MiniMe-core/releases) and download the latest APK (`minime-<tag>.apk` — one universal package for both physical devices and emulators).
...
Releases are tag-driven: push a `v*` tag on a `main` commit (e.g. `git push origin v0.1.0-rc1` / `v0.1.0`) and [`.github/workflows/android-release.yml`](.github/workflows/android-release.yml) takes over automatically → unit tests → `assembleRelease` → production signing → **dual-ABI artifact validation** → upload R8 mapping → create GitHub Release → attach `minime-<tag>.apk` → write Run Summary. RC tags (containing `-rc`) are auto-marked as prerelease.

- **Production signing**: the signing key is stored in the repository (`app/minime.jks` + `app/keystore.properties`) and used directly during CI builds — no additional Secrets configuration required.
...
- **Release page**: https://github.com/Lisir2002/MiniMe-core/releases
...
**MiniMe-core is an original project**. All code is independently designed and developed by the project team, and is not forked from or derived from any existing project.

## Acknowledgements

The following open-source projects provided reference at the technical selection and component reuse level. We acknowledge them here (all code in this project is independently implemented and does not directly incorporate source code from the above projects):

- [Termux](https://github.com/termux/termux-app) — Technical reference for Android terminal emulator and PRoot solution
- [OpenCode](https://github.com/anomalyco/opencode) — Product form reference for terminal-based AI coding tools
- [Kelivo](https://github.com/Chevey339/kelivo) — UI design reference for cross-platform LLM chat client
