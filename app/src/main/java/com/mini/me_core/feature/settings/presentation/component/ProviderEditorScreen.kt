...
fun ProviderEditorScreen(
    ...
    /** ④ 单模型覆盖：ProviderModelRow 每个模型 hasOverride 的即时快照（Flow -> State）。 */
...
// ————————————————————————————————————————————————————————————
// ③：兼容端点策略 & viewImage 守卫 两个下拉选择器
// ————————————————————————————————————————————————————————————

private fun policyDisplayName(p: DefaultPolicy): String = when (p) {
    DefaultPolicy.STRICT -> "严格模式（推荐·默认）—— 和官方收录模型走同一规则，避免 那种「全部模型都支持多模态」的副作用"
    DefaultPolicy.HEURISTIC -> "启发式模式（=）—— 按 probablyVision / probablyTools / probablyReasoning 的名字匹配自动判定"
