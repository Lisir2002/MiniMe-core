 ...
 *
 * 语义与 Room 版完全对齐（P0-1 active 互斥 / 加密失败中止 / SCHEMA 38 只写密文列）：
...
class AIProviderRepositoryV2Impl @Inject constructor(
    ...
    override suspend fun saveProvider(provider: AIProviderConfig) {
        ...
        // 先取已存在的密文，供「用户未改 API Key」时保留，避免空串覆盖。
        ...
        // V2 saveProvider 内含 P0-1 不变量（isActive 时先 deactivateAll），且为单事务。
        v2.saveProvider(
            id = provider.id,
            name = provider.name,
    ...
    /**
     * apiKey 非空 → 必须加密成功，失败抛异常中止保存（绝不写空串覆盖已有密文）；
    ...
    private suspend fun com.mini.mecore.datalayer.sqldelight.settings.Ai_providers.toDomainModel(): AIProviderConfig {
        ...
        return AIProviderConfig(
            ...
            models = modelList,
            // SCHEMA 38：selectedModel 冗余概念已合并进 defaultModel（UI 上同一语义）。
