...
class BackupManagerImpl @Inject constructor(
    ...
    private val settingsRepo: com.mini.me_core.datalayer.repository.SettingsRepository,
    private val credentialsRepo: com.mini.me_core.datalayer.repository.CredentialsRepository,
    ...
    /**
     * 统一 DAO 访问安全壳。任何 DAO 调用（Room 首次 query 时会触发 onOpen schema 校验，
    ...
    private suspend fun com.mini.mecore.datalayer.sqldelight.credentials.Git_credentials.toV2Dto(): GitCredentialDto {
    ...
    private suspend fun com.mini.mecore.datalayer.sqldelight.workspace.Remote_connections.toV2Dto(): RemoteConnectionDto {
    ...
    private suspend fun AIProviderEntity.toDto(): ProviderDto {
        // SCHEMA 38 后：Entity 中已无 apiKey/selectedModel 列。
        ...
        return ProviderDto(
            ...
            isActive = isActive,
            models = models,
            selectedModel = defaultModel, // 合并：与 defaultModel 语义一致
            isEnabled = isEnabled,
    ...
    private suspend fun ProviderDto.toEntity(): AIProviderEntity {
        // SCHEMA 38 后：Entity 只接受 encryptedApiKey，不写 apiKey/selectedModel。
    ...
    private suspend fun GitCredentialEntity.toDto(): GitCredentialDto {
        // SCHEMA 38：Entity.token 已删除，只剩 encryptedToken；createdAt/updatedAt → Ms 后缀。
