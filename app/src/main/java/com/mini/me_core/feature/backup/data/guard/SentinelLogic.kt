package com.mini.me_core.feature.backup.data.guard
 ...
 * - [PACKAGE_CHANGED]：上次运行的包名与当前不一致 —— 包名被改动（理论上被 CI 门禁拦截，运行时兜底），提示异常。
 */
 ...
 *
 * 判定顺序：
 * 1. 未初始化 → [FIRST_RUN]（全新安装，静默）
 * 2. 上次包名与当前不一致 → [PACKAGE_CHANGED]（优先于 DATA_LOST：包名变更是"全新安装"，会话数必为 0）
...
object SentinelLogic {
    ...
    fun evaluate(
        ...
        if (!meta.dataInitialized) {
            return SentinelVerdict.FIRST_RUN
        }
