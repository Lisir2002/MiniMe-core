 ...
 *
 * 宿主目录 `filesDir/minime` 即容器内 `/root/.minime`（[LinuxContainerEngine] 的 `-b` 绑定，跨 rootfs 升级不丢）。
 * 容器 [buildContainerEnv] 已设 `GIT_CONFIG_GLOBAL=/root/.minime/.gitconfig`，`.gitconfig` 里的
 * `[credential] helper = store --file=/root/.minime/git-credentials` 由 [provisionIfNeeded] 装容器时
...
class GitCredentialsFileSync @Inject constructor(
    ...
    private fun writeCredentials(creds: List<GitCredential>) {
        ...
        writeAtomically(File(minimeDir, CREDENTIALS_NAME), sb.toString())
    }
