package com.mini.me_core.feature.credentials.data.repository
 ...
 *  - 加密失败必须中止保存（），绝不写空串覆盖已有密文（Room 版 toEntity 静默降级）；
...
class CredentialRepositoryV2Impl @Inject constructor(
    ...
    /** 只从密文列解密；解密失败 → 空串 + 日志，不崩 UI（SCHEMA 38）。 */
    ...
    private suspend fun com.mini.mecore.datalayer.sqldelight.credentials.Git_credentials.toDomain(): GitCredential =
        GitCredential(
            id = id,
            host = host,
            username = username,
