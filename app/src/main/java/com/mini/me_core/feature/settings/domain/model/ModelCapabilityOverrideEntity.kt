...
/**
 * 备选方案④：单个模型级别的「多模态识图 / 工具调用 / 深度思考」三能力复选框手动覆盖。
 *
 * SCHEMA 38：改为原生复合主键 (providerType, modelId)（@Entity primaryKeys）。
 * id 字段保留（兼容旧 DAO / UI 的 composeId() 代码），但不再是 @PrimaryKey。
 *
 * 三个覆盖字段类型为 Boolean?：null 表示「用户没动，继续用启发式 / 兼容端点策略」；
 * true/false 表示用户明确覆盖为开/关。
 *
 * 解耦说明：此前错放在 feature.agent.data.local.entity，实际仅 settings 层使用，
 * 现迁移至 settings 自有 domain.model，使 settings 不再 import feature.agent.data。
 */
data class ModelCapabilityOverrideEntity(
    /** 保留用于兼容：UI 层和 Dao 仍会调用 `composeId`。后仅作冗余字段（用于日志/调试）。 */
    val id: String,
