package com.mini.me_core.feature.workspace.data.provider
 ...
 * - `projects`：工作区（各项目）；
 * - `minime`：AI 配置目录（skills/ 与 mcp.json，容器内即 `/root/.minime`）。
 *
...
class WorkspaceDocumentsProvider : DocumentsProvider() {
    ...
    /**
     * 根下唯一对外可见的子目录名白名单：工作区 `projects` 与 AI 配置 `minime`（容器内 /root/.minime，
     * 含 skills/ 与 mcp.json）。两者首次访问即创建，其余 filesDir 内部目录不列出。
     *
     * 注意：**这里绝对不能再同步调 ContainerInstaller.extractDocs(ctx())**。
    ...
    private fun exposedChildren(): List<File> {
        return listOf(
            ...
            runCatching { File(baseDir(), "minime").apply { mkdirs() } }.getOrElse { emptyList<File>().first() }
                ?: File(ctx().cacheDir, "minime_safe_fallback"),
        ).filterNotNull()
    }

    /** minSdk 26 上基类无 requireContext()，这里自取非空 context。
     *  注意：返回值非空，任何失败都转 Provider 安全的 FileNotFoundException。 */
    ...
    /**
     * 全链路沙箱校验。任何以「文件绝对路径」作为 docId 的访问都必须先过这关：
     ...
     *   3. 再额外过一遍「根下白名单子目录」：只允许 projects/ 与 minime/。
    ...
    private fun sandboxedFile(docIdOrPath: String, writeIntent: Boolean = false): File {
        ...
        // 规则 3：白名单 projects/ 与 minime/ 下的任何后代才允许访问
    ...
    override fun onCreate(): Boolean = providerSafe("onCreate") {
        ...
        runCatching {
            val c = context
            if (c != null) {
                com.mini.me_core.core.util.FileLogger.init(c)
    ...
    override fun isChildDocument(parentDocumentId: String, documentId: String): Boolean {
        // 修正：先通过 sandboxedFile 校验双方均在沙箱内，再 canonical 前缀比较。
