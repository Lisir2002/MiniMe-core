...
import com.mini.me_core.feature.agent.domain.tool.AbstractContextualTool
...
class SwitchModeTool @Inject constructor(
    ...
    override suspend fun executeWithContext(
        ...
        val sessionEntity = sessionPort.getSessionById(sessionId)
        ...
        sessionPort.upsert(sessionEntity.copy(mode = targetMode.name))

        // G-1：记录本次切换历史到数据库（持久化，可用于回溯和频率统计）
        sessionPort.insertModeSwitch(sessionId, context.mode.name, targetMode.name, reason, System.currentTimeMillis())
