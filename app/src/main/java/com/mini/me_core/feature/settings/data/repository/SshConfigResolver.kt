 ...
 * 两种路径：
 *
 * 1. `activeConnectionId != null`（新路径 / 统一数据源 / 推荐）：
 ...
 *
 * 2. `activeConnectionId == null`（老用户 fallback 路径）：
...
suspend fun RemoteConnectionSettings.resolveSshConfigOrNull(
    ...
    if (!connectionId.isNullOrBlank()) {
        ...
        return@runCatching RemoteConnectionConfig(
            ...
            port = port,
            username = username,
            auth = com.mini.me_core.feature.workspace.domain.remote.RemoteAuth.Password(password),
