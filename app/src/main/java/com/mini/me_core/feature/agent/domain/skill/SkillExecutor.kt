...
/**
 * 技能执行器（新增；重写「技能调用工具」后仅保留 PROMPT/SCRIPT 两条执行路径）。
 *
 ...
 *   （由 [com.mini.me_core.feature.agent.domain.tool.skill.LoadSkillTool] / [com.mini.me_core.feature.agent.domain.tool.skill.RunSkillScriptTool]
 *   在类型分流时给出）。
 *
...
class SkillExecutor @Inject constructor(
    ...
    private suspend fun executeScript(
        ...
        // 容器内技能目录映射：宿主 minimeDir绑定到 /root/.minime
        val containerSkillDir = "/root/.minime/skills/${skill.id}"
