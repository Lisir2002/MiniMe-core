package com.mini.me_core.feature.agent.domain.rule
 ...
 *
 * - **四级资产**：全局（`minimeDirglobal-rules.md`）/ 项目（`AGENTS.md`，已有）/
...
class RuleRegistry @Inject constructor(
    ...
    private val core = RuleAssetCore(
        globalRulesFile = { File(containerInstaller.minimeDir, GLOBAL_RULES_FILE) },
 ...
 * 未变则复用缓存，变了才重扫 —— 对齐 [com.mini.me_core.feature.agent.domain.prompt.AgentAssetCore] 做法。
 */
