package com.mini.me_core.core.data

import com.mini.me_core.datalayer.backup.SqlDelightDataProvider
 ...
 * proxy_prefs / mcp_server_prefs / app_run_meta）均已迁移到 SQLDelight AuxDb.kv_store，
...
object DataRegistryModule {
    ...
    /** AuxDb kv_store 已完全接管所有偏好项（前 DataStore 7 个文件的全部内容）。 */
    ...
    fun provideDataProviders(pool: ConnectionPool): List<DataProvider> {
        ...
        // settings / t2i / infra 已合并为 AUX 单库。
        providers += sqlDelightProviders(
            pool, LibName.AUX, V2_SETTINGS_TABLES + V2_T2I_TABLES + V2_INFRA_TABLES,
        )
        return providers
    }
