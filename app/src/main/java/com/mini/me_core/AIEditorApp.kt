...
class AIEditorApp : Application() {
    ...
    /** 数据层连接池：启动即无条件预热 AGENT 库触发结构自愈，见 [onCreate]。 */
    @Inject
    lateinit var connectionPool: ConnectionPool

    /** 长驻作用域：持续把持久化的日志等级同步到 FileLogger。
     * 附加 [CoroutineExceptionHandler]，任何子协程未捕获的异常都兜底记日志，
    ...
    override fun onCreate() {
        ...
        // ============== 关键修复：启动无条件预热 AGENT 库，铁定先于任何 UI 查询 ==============
        ...
        runCatching {
            agentDriver.executeQuery(
                ...
                { cursor ->
                    ...
                    FileLogger.i(TAG, "【诊断】agent_message 真实列: ${cols.joinToString()}")
                    QueryResult.Unit
                },
                0,
                null,
            ...
            //
            // 事故教训：此前这里手写 "SELECT agent_message.id FROM agent_message LIMIT 1"，
            // 其形态（FROM 真实表）与真实生成 SQL（FROM 匿名嵌套子查询）**不同**——
            // 手写版编译通过并打出「✅」，业务侧却照崩，诊断给出假阳性绿灯，
            // 直接误导多轮排查把精力投在「表结构缺列」的错误方向上。
            //
            ...
            AgentDb(agentDriver).agentQueries
                ...
                .executeAsList()
            FileLogger.i(TAG, "【冒烟】selectMessagesBySessionPaged 启动期执行成功 ✅")
        }.onFailure { e ->
            // 冒烟失败说明生成的 SQL 本身就不可编译/绑定——不要放行到 UI，
            // 否则就是此前那种「启动灯全绿、一进会话页就崩」。
     ...
     *
     *  关键修正：此前 FileLogger 把 CRASH 行通过 ioExecutor 异步落盘，
