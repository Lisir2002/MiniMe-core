...
/**
 * 数据持久化守卫：release 变体的 applicationId 必须恒为 `com.mini.me_core`（rebrand MiniMe-core）。
 *
 * 背景：包名（applicationId）四次变更——`com.aicodeeditor` → `com.deep.rcode` →
 * `com.R.codecore` → `com.mini.me_core`（rc11，MiniMe-core 品牌迭代，显式 rebrand）。
 * 每次变更在 Android 眼里都是**完全不同的 App**，安装新包名是一次"全新安装"——本仓库当前 MiniMe-core 品牌迭代（com.mini.me_core → com.mini.me_core）同样遵守这个规则，其私有数据目录
 * （/data/data/<新包名>/）为空 → 旧包的全部历史对话"消失"，只剩新装后自动创建的那一条会话。
 * 这是历史上"每次更新历史对话清空"的直接根因。
 *
 * rc11 品牌迭代为用户显式决策（当时处于内测阶段，正式用户尚未引入），此后本测试继续把
 * release applicationId 锁死在 `com.mini.me_core`，防止未来 rebrand 误改包名再次造成数据丢失。
