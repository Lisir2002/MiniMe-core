...
class HostKeyManager @Inject constructor(
    ...
    private fun verifyKey(hostname: String, port: Int, key: PublicKey): Boolean {
        ...
        val entries = if (file.exists()) {
            try {
                ...
                MiniMeLog.w(TAG, "读取 known_hosts 失败，将重建: ${e.message}")
                emptyList()
            }
        } else {
            emptyList()
        ...
        if (existing.isEmpty()) {
            // 首次连接：接受并保存密钥
            MiniMeLog.i(TAG, "首次连接 $hostKey，接受并保存主机密钥 ($keyType)")
            try {
                // 追加到 known_hosts 文件
                val line = "$hostKey $fingerprint\n"
                file.appendText(line)
            } catch (e: Exception) {
                MiniMeLog.w(TAG, "保存主机密钥失败: ${e.message}")
            }
            return true
        }
        ...
        if (matched) {
            MiniMeLog.v(TAG, "主机密钥验证通过: $hostKey")
            return true
        }
        ...
        MiniMeLog.w(TAG, "主机密钥不匹配! host=$hostKey savedType=$savedType savedKey=$savedKey")
    ...
    fun clearAllKeys() {
        if (knownHostsFile.exists()) {
            ...
            MiniMeLog.i(TAG, "已清除所有 saved host keys")
        }
    }
    ...
    fun clearHostKey(hostname: String, port: Int = 22) {
        ...
        MiniMeLog.i(TAG, "已清除主机密钥: $hostKey")
    }
}
