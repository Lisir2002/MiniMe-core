 ...
 * 2. 启动构造：纯内存赋值，不碰 IO/Keystore（遵循 启动期 ANR 规避约定）。
 * 3. 所有 encrypt/decrypt 必须在 Dispatchers.IO 线程调用（启动阻塞修正）。
...
class ZthSensitiveColumnCrypto @Inject constructor(
    ...
    suspend fun ensureReady() {
        ...
        initDone.set(true)
        MiniMeLog.i(TAG, "ZTH 敏感列加密就绪（复用 CredentialEncryptor V2 架构，AES-256-GCM）")
    }
