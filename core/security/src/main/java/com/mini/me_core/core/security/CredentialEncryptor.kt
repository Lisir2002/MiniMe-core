...
class CredentialEncryptor @Inject constructor(
    ...
    /** 加密状态持久化端口（反转自 datalayer.WorkspaceRepository，由 :app 适配注入）。 */
    private val stateStore: CredentialStateStore,
    /** 安全审计端口（反转自 feature.workspace.RemoteAuditLogRepository，由 :app 适配注入）。 */
    private val auditRecorder: SecurityAuditRecorder,
    /**
     ...
     * 启动后首次加密/解密前必须调用；幂等。
     *
     * **关键修正：** 方法体内强制 [withContext(Dispatchers.IO)] 包裹全部
     ...
     *
     * ============= 启动闪退根因级修复 =============
     * 此前 ExecutionModeRepository 不用 CredentialEncryptor；此后它在
        ...
        initMutex.withLock {
            ...
            val result = runCatching {
                ...
                if (existing == null) {
                    ...
                    dekCached = dek
                    MiniMeLog.i(TAG, "首次初始化完成：MasterKey + DEK 已生成")
                    return@runCatching InitResult.OK
                }

                // 修复：MasterKey 匹配校验改为「直接 unwrap DEK」。
                ...
                //
                // 增强：unwrap 失败时不再直接返回 TAMPERED 让初始化永久失败（那样后续所有
                ...
                if (!existing.migratedFromV1) {
                    MiniMeLog.i(TAG, "检测到 V1 数据未迁移，pending 后台 V1toV2MigrationWorker")
                }
            ...
            when {
                result.isSuccess && result.getOrNull() == InitResult.OK -> {
                    ...
                    MiniMeLog.e(TAG, note, result.exceptionOrNull())
                    MiniMeLog.flush("ERROR", TAG, note, result.exceptionOrNull())
                }
            }
        }
    }
     ...
     *
     * 修正：强制 IO 线程，避免 Flow 收集线程（mapLatest 内调用时）阻塞首帧。
     */
    ...
    suspend fun scheduleRotateDek(): OperationResult<RotationReport> = withContext(Dispatchers.IO) {
        return@withContext try {
            ...
            MiniMeLog.e(TAG, "DEK 轮换失败（已回滚，旧 DEK 继续生效）: ${e.message}", e)
            auditRecorder.record(
                category = RemoteAuditCategory.CREDENTIAL,
                action = RemoteAuditAction.CRED_ROTATE_DEK,
                success = false,
    ...
    suspend fun setBiometricRequired(required: Boolean): OperationResult<Unit> =
        withContext(Dispatchers.IO) {
            return@withContext try {
                ...
                MiniMeLog.i(TAG, "生物识别保护切换为: $required")
    ...
    suspend fun emergencyResetMasterKey(): ResetReport = withContext(Dispatchers.IO) {
        ...
        MiniMeLog.i(TAG, "紧急重置 MasterKey 完成")
    ...
    // ── V2 映射 ──────────────────────────────────────────────────────
    // SQLDelight 行 ↔ CredentialEncryptionStateEntity 的映射已下沉到 :app 的
    // CredentialStateStore 适配器（core/security 不再依赖 datalayer 生成类型）。
