 ...
 *
 * DEK 保护改用 AES/GCM ENCRYPT/DECRYPT（而非 WRAP/UNWRAP）。
...
class DEKManager private constructor() {
    companion object {
        ...
        private const val MASTERKEY_ALIAS = "minime_credential_masterkey"
    ...
    fun getOrCreateMasterKey(biometricRequired: Boolean = false): SecretKey {
        ...
        if (existingEntry != null) {
            // 根因修复（DEK 未初始化 / Incompatible purpose）：
            ...
            MiniMeLog.w(TAG, "已存在 MasterKey 不支持 ENCRYPT/DECRYPT（用途不兼容），删除并重建")
            keyStore.deleteEntry(MASTERKEY_ALIAS)
            cachedDek = null
        }

        MiniMeLog.i(TAG, "生成新的 MasterKey（biometricRequired=$biometricRequired）")
        val keyGenerator = KeyGenerator.getInstance(
            KeyProperties.KEY_ALGORITHM_AES,
            ANDROID_KEYSTORE
        )
        // MasterKey 授权 ENCRYPT|DECRYPT，用 AES/GCM 加密 DEK 原始字节（标准做法）。
    ...
    /**
     * 校验 MasterKey 是否支持 ENCRYPT/DECRYPT 用途。
    ...
    fun deleteMasterKey() {
        try {
            ...
            keyStore.deleteEntry(MASTERKEY_ALIAS)
            cachedDek = null
            MiniMeLog.i(TAG, "已删除 MasterKey")
        } catch (e: Exception) {
            MiniMeLog.e(TAG, "删除 MasterKey 失败", e)
        }
    }
     ...
     *
     * **根因修复：** 旧实现用 `Cipher.WRAP_MODE` 包装 DEK，但 Android Keystore 的
     ...
     *
     * **** 与 [wrapDek] 配套，使用 AES/GCM DECRYPT_MODE，从
