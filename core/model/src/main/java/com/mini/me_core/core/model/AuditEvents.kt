...
/**
 * 远程审计日志分类常量（跨 feature 共享，下沉到 :core:model）。
 *
 * 迁移自 feature.workspace.domain.RemoteAuditCategory：这些是纯字符串常量，
 * 被 :core:security（凭据轮换审计）、feature.agent（技能执行审计）、feature.settings（审计日志页）
 * 共同使用。放在纯 Kotlin 的 :core:model 后，:core:security 不再反向依赖 feature.workspace，
 * 从而打破 core/security ↔ feature/workspace 的包级循环。
 */
