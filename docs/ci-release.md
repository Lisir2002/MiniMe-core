...
2. **build** → `:app:testReleaseUnitTest`（发版质量门禁）→ `:app:assembleRelease`（签名密钥已入库：`app/minime.jks` + `app/keystore.properties`，构建时直接使用）→ **正式签名**构建 APK 到 `app/build/outputs/apk/release/app-release.apk` → `Rename APK` 重命名为 `dist/minime-<tag>.apk`（**双 ABI 通用包**，重命名同时做 ABI 校验：`lib/` 必须同时含 `arm64-v8a` 与 `x86_64`）
